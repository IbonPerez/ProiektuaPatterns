...
public class TableMaiteGUI extends JFrame {
	...
	public TableMaiteGUI() {
		...
		columnNamesZeini= new String[]{"Event","Question","EventDate","Bet"};
		...
		tableModelZein.setColumnCount(4);
		User u = facade.getUser("beki@gmail.com");
		UserAdapter usadap = new UserAdapter(u);
		
		for (int i=0; i<usadap.getRowCount();i++){
			Vector<Object> row = new Vector<Object>();
			
			row.add(usadap.getValueAt(i, 0));
			row.add(usadap.getValueAt(i, 1));
			row.add(usadap.getValueAt(i, 2));
			row.add(usadap.getValueAt(i, 3));
...
//						return;
	...
	public TableMaiteGUI(final String text) {
		setTitle(ResourceBundle.getBundle("Etiquetas").getString("ErabiltzaileaErreplikatuGUI.this.title")); //$NON-NLS-1$ //$NON-NLS-2$
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(100, 100, 487, 354);
		contentPane = new JPanel();
		...
		tableModelZein.setDataVector(null, columnNamesZeini);
		tableModelZein.setColumnCount(usadap.getColumnCount());
